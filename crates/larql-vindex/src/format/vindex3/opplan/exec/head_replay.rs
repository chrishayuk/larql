...
pub fn replay_attention_mixture<B: PlanBackend + ?Sized>(
    ...
    let raw_attention_output = backend.project(ProjectCall {
        weight: operands.w_o.slice(),
        out_dim: hidden,
        in_dim: width,
        x: &concat,
    })?;
    if raw_attention_output.len() != hidden
    ...
    let mut applied_delta = match &prepared_layer.post_attention {
        Some(norm) => norm.apply(backend, &raw_attention_output),
        None => raw_attention_output.clone(),
    };
    scale_residual_delta(layer_plan.residual_scale, &mut applied_delta);
...
pub fn replay_attention_heads<B: PlanBackend + ?Sized>(
    ...
    let mixture = replay_attention_mixture(
        plan,
        prepared,
        backend,
        layer,
    ...
    let mut contributions = Vec::with_capacity(op.num_q_heads);
    let mut contribution_norms = Vec::with_capacity(op.num_q_heads);
    let mut reconstructed = vec![0.0f64; hidden];
    for (head, values) in head_values.iter().enumerate() {
        let mut isolated = vec![0.0f32; width];
        isolated[head * op.head_dim..(head + 1) * op.head_dim].copy_from_slice(values);

...
static REQUESTS: Mutex<Vec<super::prefetch::PrefetchReport>> = Mutex::new(Vec::new());
...
/// Record one routed call's request shape: the ranges its prefetch
/// covered, the requests it issued for them and the bytes those asked
/// for. A no-op unless a capture is open.
pub fn record_requests(report: super::prefetch::PrefetchReport) {
    if !ACTIVE.load(Ordering::Acquire) {
        return;
    }
    REQUESTS.lock().expect("request capture lock").push(report);
}

/// Every routed call's request shape since the capture opened.
pub fn take_requests() -> Vec<super::prefetch::PrefetchReport> {

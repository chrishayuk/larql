...
impl PreparedOperands {
    ...
    pub fn select_output_head(&self, token_ids: &[u32]) -> Result<SelectedOutputHead, VindexError> {
        ...
        let vocab = op.projection.shape[0];
        ...
        let projection = match weight.slice().rows(vocab, self.hidden)? {
            WeightRows::F32(values) => {
                let mut selected = Vec::with_capacity(token_ids.len() * self.hidden);
                for &token in token_ids {
                    let start = token as usize * self.hidden;
                    selected.extend_from_slice(&values[start..start + self.hidden]);
                }
            ...
            WeightRows::Q8 {
                ...
                let scales_per_row = self.hidden.div_ceil(block);
                let sums_per_row = self.hidden.div_ceil(SUM_BLOCK);
                let mut selected_codes = Vec::with_capacity(token_ids.len() * self.hidden);
                ...
                for &token in token_ids {
                    ...
                    selected_codes
                        .extend_from_slice(&codes[row * self.hidden..(row + 1) * self.hidden]);
        ...
        Ok(SelectedOutputHead {
            token_ids: token_ids.to_vec(),
            projection,
            hidden: self.hidden,
            multiplier: op.multiplier,
            softcapping: op.softcapping,
        })
    }

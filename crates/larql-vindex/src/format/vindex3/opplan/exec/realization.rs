...
pub enum MappedAccess {
    ...
    /// pieces.
    Coalesced,
    /// `Coalesced`, asked only for the runs a residency probe finds are
    /// not already in memory — the access conditioned on what the token
    /// would have found anyway, so a warm token issues no request at all.
    Gated,
}

impl MappedAccess {
    pub const ALL: [MappedAccess; 5] = [
        MappedAccess::Demand,
        MappedAccess::Advise,
        MappedAccess::Touch,

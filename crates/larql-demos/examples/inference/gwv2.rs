...
use larql_vindex::format::vindex3::opplan::exec::{
    ...
    intervene::{Address, Intervention, InterventionPlan, VectorProvenance, vector_sha256},
...
impl StepObserver for Capture {
    ...
    fn attention_head(&mut self, layer: usize, record: AttentionHeadRecord<'_>) {
        ...
        let values = self
            .positions
            .iter()
            .map(|&p| record.source_values[p].clone())
            .collect();
        ...
        if layer == 24 {
            ...
            self.keys = record.source_keys.to_vec();
            self.values = record.source_values.to_vec();
        }
    }
    fn carrier_write(&mut self, record: CarrierWriteRecord<'_>) {
        if record.position != self.target {

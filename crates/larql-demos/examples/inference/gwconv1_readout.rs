//! Frozen GW-CONV-1 selected-head readout over sealed pre-write carriers.
use super::gwsup1_readout::{
    artifact, checked_bytes, diagnostic, f32_rows, file_sha, l2, write_json_line,
};
use super::*;
use std::fs::{File, OpenOptions};
use std::io::{BufRead, BufReader, BufWriter, Write};

const PREREG_SCHEMA: &str = "larql.gwconv1.preregistration.v1";
const CANDIDATE_SCHEMA: &str = "larql.gwsup1.candidates.v1";
const OUTPUT_SCHEMA: &str = "larql.gwconv1.before-readout-manifest.v1";

/// `--gwconv1-readout CONTAINER PREREG CANDIDATES SEALED_MANIFEST OUTPUT_DIR`
pub(super) fn run(args: &[String]) -> Result<()> {
    if args.len() != 5 {
        return Err("Usage: observatory_record --gwconv1-readout CONTAINER PREREG CANDIDATES SEALED_MANIFEST OUTPUT_DIR".into());
    }
    ...
    let output_manifest = output.join("before-readout-manifest.json");
    if output_manifest.exists() {
        return Err(format!("GW-CONV-1 output is already sealed at {}", output.display()).into());
    }

    let prereg: Value = serde_json::from_slice(&std::fs::read(prereg_path)?)?;
    if prereg["schema"] != PREREG_SCHEMA || prereg["status"] != "frozen_pre_before_readout" {
        return Err("GW-CONV-1 preregistration is not frozen".into());
    }
    let prereg_identity = prereg["preregistration_sha256"]
        .as_str()
        .ok_or("GW-CONV-1 identity missing")?;
    ...
    {
        return Err("candidate authority does not match GW-CONV-1".into());
    }
    ...
    if token_ids.len() != 126 {
        return Err("GW-CONV-1 candidate vocabulary no longer has 126 tokens".into());
    }

    let sealed: Value = serde_json::from_slice(&std::fs::read(sealed_path)?)?;
    if prereg["authorities"]["sealed_gw0"]["sha256"].as_str() != Some(&file_sha(sealed_path)?)
    ...
    {
        return Err("sealed GW-0 authority does not match GW-CONV-1".into());
    }
    ...
    {
        return Err("prepared image differs from the frozen GW-CONV-1 authority".into());
    }
    ...
    let logits_path = output.join("candidate-before-logits.f32");
    let rows_path = output.join("before-rows.jsonl");
    ...
    for line in BufReader::new(File::open(&census_path)?).lines() {
        let row: Value = serde_json::from_str(&line?)?;
        let edge_id = row["edge_id"].as_str().ok_or("sealed edge ID missing")?;
        let record: Value = serde_json::from_slice(&checked_bytes(
            &artifact_root,
            artifact(&row, "execution_record")?,
        )?)?;
        let sites = record["sites"]
            .as_array()
            .ok_or("execution sites missing")?;
        if sites.len() != 68 || record["edge_id"] != row["edge_id"] {
            return Err(format!("{edge_id}: execution site coverage mismatch").into());
        }
        let descriptor = artifact(&row, "carrier-before")?;
        if descriptor["shape"] != json!([68, ops.hidden()]) {
            return Err(format!("{edge_id}: before-carrier shape changed").into());
        }
        let carriers = f32_rows(
            &checked_bytes(&artifact_root, descriptor)?,
            68,
            ops.hidden(),
        )?;
        let mut site_rows = Vec::with_capacity(68);
        for (site_index, (site, state)) in sites
            .iter()
            ...
            .enumerate()
        {
            ...
            if logits.len() != token_ids.len() || logits.iter().any(|value| !value.is_finite()) {
                return Err(
                    format!("{edge_id}: invalid before readout at site {site_index}").into(),
                );
            }
            if parity.is_none() {
        ...
        if row_count.is_multiple_of(25) || row_count == 426 {
            eprintln!("GW-CONV-1 before readout {row_count}/426");
        }
    }
    if row_count != 426 {
    ...
    drop(logits_writer);
    drop(rows_writer);
    let expected_bytes = row_count * 68 * token_ids.len() * 4;
    if std::fs::metadata(&logits_path)?.len() != expected_bytes as u64 {
        return Err("before-logit artifact has the wrong byte length".into());
    }
    let parity = parity.ok_or("no before-readout parity witness")?;
    if parity["bit_mismatches"] != 0 || parity["max_abs_difference"] != 0.0 {
        return Err(format!("selected/full before-head parity failed: {parity}").into());
    }
    let manifest = json!({
        "schema": OUTPUT_SCHEMA,
        "status": "before_readout_complete_pre_site_selection",
        ...
        "artifacts": {
            "logits": {"path": "candidate-before-logits.f32", "dtype": "f32-le", "bytes": expected_bytes, "sha256": file_sha(&logits_path)?},
            "rows": {"path": "before-rows.jsonl", "rows": row_count, "sha256": file_sha(&rows_path)?},
        },
        "selected_full_head_parity": parity,
        "prompts_reexecuted": false,
        "site_profile_inspected_before_readout": false,
    });

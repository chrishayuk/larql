...
pub(super) fn run_residency_curve<B: PlanBackend>(
    ...
    for pass in 1..=repeat.max(1) {
        ...
        for step in 0..new_tokens {
            ...
            if step == 0 {
                ...
                let request_shape = (!requests.is_empty()).then(|| {
                    requests
                        .iter()
                        .fold((0usize, 0usize), |(r, q), (ranges, reqs)| {
                            (r + ranges, q + reqs)
                        })
                });
        ...
        if let Some(sample) = token_sample {
            ...
            if let Some((ranges, requests)) = sample.request_shape {
                println!("  token 1: prefetch covered {ranges} ranges in {requests} request(s)");
            }
...
struct PassSample {
    ...
    /// Ranges the token's prefetches covered and the requests they issued.
    request_shape: Option<(usize, usize)>,
}

#!/usr/bin/env python3
"""LEGACY Q-BANK-1 runner, frozen 2026-09-23. Kept only to reproduce results
it produced: CPU-5, CPU-6 and the QB-2 acceptance run through it.

It computes its own numbers (bits over the full vocabulary, no validity
proofs), and it compares arithmetic modes selected by ENVIRONMENT variables
across two processes, which the MEASURE-PLAN-1 procedure cannot express,
because it runs both arms in one process. New work uses `run_bank.py`, a
client of `larql vindex3 measure` (docs/measure-plan-1.md), with arithmetic
modes declared as registered backends. Do not extend this file.

Q-BANK-1 runner: characterise a compiled representation against BF16.

    python3 run_bank_legacy.py reference <container> <tokenizer.json> <out-dir> [--backend metal]
    python3 run_bank_legacy.py compare   <container> <out-dir> [--backend ... --source stored] [--keep] [--per-prompt]
    python3 run_bank_legacy.py report    <out-dir>

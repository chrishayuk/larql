...
Shipped-work history has moved to [`CHANGELOG.md`](CHANGELOG.md); the live
status board is [`ROADMAP_STATUS.md`](ROADMAP_STATUS.md). Engine numbers
quoted while planning come from the funnel docs' gate logs, which are the
source of truth — not from a summary in this file.

---

## Codebase hardening & cleanup

Open remediation items from the whole-codebase reviews (2026-05-28,
2026-06-12), the DEC-readiness review (2026-07-22), the vindex + walk-FFN
review (2026-07-30), the extraction tensor-coverage audit (2026-07-31), the
compute-layer hygiene review (2026-08-05), and the standing cleanup /
consolidation track now live in
[`docs/hardening-backlog.md`](docs/hardening-backlog.md). Several are still
open P0s — treat that file as backlog, not archive.

The closed K3 / GPT-OSS serving results that used to sit in this section
(R1 P4/P5, the cross-expert transport codec census, the serving-format
ladder, R1 Gate B) are in [`CHANGELOG.md`](CHANGELOG.md); their write-ups
remain in [`docs/k3-funnel.md`](docs/k3-funnel.md).
...
The principle to lock in: **text is one interoperability layer, not
...
generated ids, residual state, KV state — with English text reserved for
when text genuinely is the cheapest interchange format.

The programme has its own gate log now:
[`docs/fuse-funnel.md`](docs/fuse-funnel.md) — the five compatibility
levels, the FUSE-0..4 ladder (plus FUSE-1.5 at the additive input seam
and FUSE-2.5 on backbone depth necessity), the MOSS checkpoint facts that
make each rung well-posed, what fusion does and does not buy, and the
tracked prior art.

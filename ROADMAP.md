...
| C2 | WalkFfn as **primary** CPU decode path (not research-only mode) | larql-inference | **partial, RE-SCOPED 2026-08-02 — do not promote WalkFfn's dynamic sparse-gather mode as default.** R4 zero-out (2026-08-01, [[project_r4_zeroout_sparse_ffn]]) + LA-6/LA-7 (2026-08-02, [[project_vindex2_la_programme]], `docs/vindex2-la.md` §2l) independently closed dynamic feature/row/block-selective FFN execution as a throughput lever: the isolated sparse kernel is genuinely 1.29× faster, but the REAL decode loop runs at ~0.15× baseline (page/scheduling overhead dominates) and the sparse kernel captures only 23–40% of its theoretical row reduction. `WeightFfn::forward` (dense) stays the correct default. If C2 proceeds, it should target WalkFfn's **static/compiled** execution mode (a fixed, precompiled route with no per-token gather) — not the dynamic gate-KNN/oracle-routed mode this item originally specified, which is now a known regression. | Currently `WeightFfn::forward` is the dense fallback; switch the default for vindex-loaded models to `WalkFfn`. Bench numbers required. Cross-references CPU MoE work in C1. |
...
(C2 re-scoped 2026-08-02 — see its row above. Its slot in this sequence assumed the dynamic sparse-gather mode; that mode is now a closed regression (R4 + LA-6/LA-7). C2's dense-fallback-stays-default status doesn't block C7/C12/C3/C4 downstream, so the sequence order itself is unaffected, but C2 as an item needs new scope before it can graduate past "partial.")

...
**Status:** DESIGN DRAFT — not preregistered, sealed, or executed  
...
state. Zero carrier and zero-head inputs are outside the registered execution
surface and enter neither selection nor adjudication.
...
The final preregistration must freeze one simultaneous one-sided
...
controls. `scripts/gwstate1_preregister.py` constructs and validates an
unsealed machine protocol; `scripts/gwstate1_select.py` and
`scripts/gwstate1_adjudicate.py` implement the later stage seals. No STATE-1
context outcome has been produced. The draft and runner still require joint
review before a protocol/executable seal or replay.
